...
public class FoodClub implements ConfigProperties {
	...
	private boolean bet(FoodClubBets bets) throws HttpHelperException {
	...
	private FoodClubOdds parseCurrentOdds() throws HttpHelperException {
		Document doc = Jsoup.parse(loadFoodClubPage().getContent().get());
		String amountStr = DomUtils.getContent(doc).select(":root > p").get(3).text();
		int maxBet = Integer.parseInt(amountStr.split("up to ")[1].split(" NeoPoints")[0]);
		Element formTable = DomUtils.getContent(doc).select(":root > center > form > table").get(0);
	...
	private HttpResponse placeBet(FoodClubPostForm form) throws HttpHelperException {
	...
	private HttpResponse loadFoodClubPage() throws HttpHelperException {
		return helper.get("/pirates/foodclub.phtml")
				.addQueryParameter("type", "bet")
				.send();
	}
	
	private FoodClubBets getLeftysBetInfo() throws HttpHelperException {
		Document doc = Jsoup.parse(loadLeftysPage().getContent().get());
	...
	private HttpResponse loadLeftysPage() throws HttpHelperException {
