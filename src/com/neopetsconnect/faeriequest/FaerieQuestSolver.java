...
public class FaerieQuestSolver implements ConfigProperties {
	...
	public void solveQuest() throws HttpHelperException, FaerieQuestBlockedException {
		...
		try {
			...
			Map<Item, List<ShopItem>> bought = 
					new ShopWizard(helper).buyIfWorthIt(Arrays.asList(item.get()), 
							FAERIE_QUEST_MAX_COST, SHOP_WIZ_SEARCH_TIMES);
			if (bought.isEmpty()) {
			...
			boolean updatePriceResult = 
					new MyShop(helper).changePrice(item.get().getName(), 
	...
	private Optional<Item> talkToFaerie() throws HttpHelperException, FaerieQuestBlockedException {
		Element content = DomUtils.getContent(loadFaerieQuestPage());
		Element questContent = content.getElementById("fq2");
		String faerie = questContent.getElementsByClass("description_top").first()
				.text().replaceAll("(You are currently on a quest for a |Faerie|\\.)", "")
				.toLowerCase()
				.trim();
		...
		if (!item.isEmpty()) {
			...
			String ck = content
					.getElementById("fq2_form").select(":root > input[name='ck']")
					.first().val();
			...
			String description = questContent.getElementsByClass("description_bottom")
					.first().text();
			throw new FaerieQuestBlockedException(description);
		}
	}
	
	private HttpResponse loadAbandonQuest(String ck) throws HttpHelperException {
		return helper.post("/quests.phtml")
				...
				.addFormParameter("action", "abandon")
				.addFormParameter("ck", ck)
				.send();				
	}
	
	private HttpResponse loadCompleteQuest(String ck) throws HttpHelperException {
		return helper.post("/quests.phtml")
				.addHeader(Headers.referer("http://www.neopets.com/quests.phtml"))
				.addHeader(Headers.contentType(HttpContentType.APP_X_WWW_FORM_ENCODED))
				.addFormParameter("action", "complete")
				.addFormParameter("ck", ck)
				.send();				
	}

	private HttpResponse loadFaerieQuestPage() throws HttpHelperException {
