...
public class CaptchaSolver {
  ...
  static {
    try {
      bases = Files.list(Paths.get(basesPath)).filter(p -> p.toFile().isFile()).map(p -> {
        try {
  ...
  public static int[] solve(HttpHelper helper, String url) {
    Logger.out.logTimeStart("", TimeUnits.SECONDS);
  ...
  public static BufferedImage getBufferedImage(ColorImage colorImage) {
    BufferedImage bufferedImage = new BufferedImage(colorImage.getWidth(), colorImage.getHeight(),
        BufferedImage.TYPE_INT_RGB);

    for (int i = 0; i < colorImage.getWidth(); i++) {
      for (int j = 0; j < colorImage.getHeight(); j++) {
        bufferedImage.setRGB(i, j, ColorHelper.convertToRgb(colorImage.getRed(i, j),
            colorImage.getGreen(i, j), colorImage.getBlue(i, j)));
      }
    }
  ...
  static ColorImage toBinary(ColorImage colorImg) {
    ...
    double v = FilterUtils.applyMask(grayImg.getBands().get(0), avgMask, grayImg.getWidth() / 2,
        grayImg.getHeight() / 2);
  ...
  private static ColorImage findBase(ColorImage colorImage, List<ColorImage> bases) {
    return bases.stream().filter(base -> base.getWidth() == colorImage.getWidth()
        && base.getHeight() == colorImage.getHeight()).collect(Collectors.toMap(base -> {
        ...
        }, x -> x)).entrySet().stream().sorted(Map.Entry.comparingByKey()).findFirst().get()
        .getValue();
  }
  ...
  private static double subtract(ColorImage img1, ColorImage img2) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      new SubstractImageTransformation(band1).transform(band2);
      total += Arrays.stream(band2.pixels)
          .mapToDouble(row -> Arrays.stream(row).map(v -> Math.abs(v)).sum()).sum();
    }
    return total;
  }
