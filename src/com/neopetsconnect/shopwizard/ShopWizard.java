...
public class ShopWizard implements ConfigProperties {
	...
	public List<ShopItem> buyItem(List<ShopItem> shopItems, int amount) 
			throws HttpHelperException, ItemNotFoundException {
	...
	public int findPrice(String name, int iterations) 
			throws HttpHelperException, ItemNotFoundException, ShopWizardBannedException, FaerieQuestException {
		validateShopWizard();
	...
	private Map<Item, List<ShopItem>> findItems(List<Item> items, int searchTimes) {
		return items.stream().collect(Collectors.toMap(item -> item, item -> {
			...
			} catch (ItemNotFoundException | HttpHelperException |
					ShopWizardBannedException | FaerieQuestException e) {
	...
	private List<ShopItem> findItem(String name) 
		...
		if (!tables.isEmpty()) {
			...
			List<Element> itemElems = tables.get(1).select(":root > tbody > tr").stream()
					.collect(Collectors.toList());
			itemElems.remove(0);
	...
	private void validateShopWizard() 
			throws HttpHelperException, FaerieQuestException, ShopWizardBannedException {
		Element content = DomUtils.getContent(loadShopWizard());
	...
	public ShopItem buyFromUser(String username, Item item) throws HttpHelperException {
		Element content = DomUtils.getContent(loadUserShop(username));
		...
		String link = itemElem.select(":root > a").first().attr("href").replaceAll("http://www.neopets.com", "");
	...
	private HttpResponse loadBuyPageFromOwner(String link, String username) throws HttpHelperException {
		if (!link.startsWith("/")) {
			link = "/" + link;
		}
	...
	private HttpResponse loadShopWizard() throws HttpHelperException {
		return helper.get("/market.phtml")
				.addQueryParameter("type", "wizard")
				.send();
	}
	
	private HttpResponse loadUserShop(String username) throws HttpHelperException {
		return helper.get("/browseshop.phtml")
				.addQueryParameter("owner", username)
				.send();
	}
